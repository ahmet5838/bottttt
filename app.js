...
app.use(session({
secret: 'EQeO9nsCaiY0cCn2NPRe7vXFodz7LxXi',
resave: false,
...
app.get("/verificationss/:botID", checkAuth, (req, res) => {

renderTemplate(res, req, "verificationss.ejs")
});
...
app.post("/verificationss/:botID", checkAuth, (req, res) => {
...
res.redirect("/yetkili")
  ...
  db.set(`sertifikak.${owneridd}.durum`, true)
    ...
    client.channels.get(client.ayarlar.kayıt).send(`<@${db.fetch(`botlar.${id}.sahipid`)}> adlı kullanıcının ${db.fetch(`botlar.${id}.isim`)} adlı botun sertifikası Onaylandı. `)
